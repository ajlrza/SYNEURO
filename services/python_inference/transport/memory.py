    ...
    for memory in memory_holder:
        ...
        while (neuron_activated != True):
            ...
            if (neuron_status[random_available_neuron] == "Active"):
                ...
                if (len(random_available_neuron) in int_to_string_map_scale.keys()):

                    get_length = int_to_string_map_scale[len(random_available_neuron)]

                    if (get_length == 3):

                        stringify = str(random_available_neuron)
                        int_tmp_list = []

                        for number in stringify:
                    ...
                    get_double_digit = int_to_string_map_double[int(int_tmp_list[1] + "0")]
                    int_tmp_list[0] = get_double_digit
